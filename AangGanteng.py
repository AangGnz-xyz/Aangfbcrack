...
logo = ''' \033[0;96m  __  ______  ____   
 \033[0;96m  /  |/  / _ )/ __/ \033[0m|| Author : Aang Ganteng
 \033[0;96m  / /|_/ / _  / _/ \033[0m|| Guthub : Github.com/AangGnz-xyz
 \033[0;96m  /_/  /_/____/_/ \033[0;91mv2.1  \033[0m|| Fb        : Facebook.com/clubfunbike
...
def yayanxd():
    ...
    try:
        ...
        print ' %s*%s mohon untuk menggunakan script ini sewajarnya saja, kami tidak bertanggung jawab jika script ini disalah gunakan...'%(O,N);time.sleep(2)
        print ' %s*%s Contact WhatsApp: 089524163441'%(O,N);time.sleep(2)
...
def moch_yayan():
    ...
    print ' [%s8%s]. Setting user agent'%(O,N);time.sleep(0.03)
    print ' [%s9%s]. Info %sscript%s'%(O,N,O,N);time.sleep(0.03)
    print ' [%s0%s]. Log out (%shapus token%s)'%(M,N,M,N);time.sleep(0.03)
    pepek = raw_input('\n [*] menu : ')
    if pepek == '':
        print '\n %s[%s×%s] JANGAN KOSONG NGENTOT!!'%(N,M,N);time.sleep(2);moch_yayan()
    elif pepek in['1','01']:
        teman(kontol)
    elif pepek in['2','02']:
        publik(kontol)
...
def teman(kontol):
    ...
    try:
        ...
        print ' [%s•%s] salin file hasil dump👉 ( %s%s%s )'%(O,N,M,cin,N)
        print 50 * '-'
...
class __crack__:
    ...
    def plerr(self):
        try:
            ...
            print '\n %s[%s×%s] File [%s%s%s] tidak ada, dump id dulu lah bro cek nomor 1 sampai 4'%(N,M,N,M,self.apk,N);time.sleep(3)
            raw_input('\n  %s[ %skembali%s ]'%(N,O,N));moch_yayan()
        ...
        if ___yayanganteng___ in ('Y', 'y'):
            ...
            while True:
                ...
                if pwek == '':
                    print '\n %s[%s×%s] kata sandi jangan kosong kentod_-'%(N,M,N)
                elif len(pwek)<=5:
                    print '\n %s[%s×%s] kata sandi minimal 6 karakter'%(N,M,N)
                else:
                    def __yan__(ass=None): # ass => aang sayang saskiya :3
                        cin = raw_input('\n [*] method : ')
                        if cin == '':
                            print '\n %s[%s×%s] jangan kosong ngentott'%(N,M,N);self.__yan__()
                        elif cin == '1':
    ...
    def __api__(self, user, __yan__):
        ...
        for pw in __yan__:
            ...
            if response.status_code != 200:
                sys.stdout.write('\r %s[%s!%s] IP terblokir hidupkan mode pesawat 5 detik saja'%(N,M,N)),
                sys.stdout.flush()
                loop +=1
                self.__api__()
    ...
    def __pler__(self):
        yan = raw_input('\n [*] method : ')
        if yan == '':
            print '\n %s[%s×%s] jangan kosong ngentott'%(N,M,N);self.__pler__()
        elif yan in ('1', '01'):
            print '\n [%s+%s] hasil OK disimpan ke -> results/OK-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print ' [%s+%s] hasil CP disimpan ke -> results/CP-%s-%s-%s.txt'%(O,N,ha, op, ta)
            print '\n [%s!%s] anda bisa mematikan data selular untuk menjeda proses crack\n'%(M,N)
